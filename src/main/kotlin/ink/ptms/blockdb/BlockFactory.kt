...
object BlockFactory {
    ...
    private fun Int.roundToRegion(): Int {
        return this shr 5
    }

    @Schedule(period = 3000, async = true)
    @Awake(LifeCycle.DISABLE)
    internal fun save() {
        worlds.forEach { (_, world) -> world.save() }
    }

    @Schedule(period = 1000, async = true)
    internal fun check() {
    ...
    data class World(val name: String) {
        ...
        fun loadRegion(regionX: Int, regionZ: Int, reload: Boolean = false): WorldRegion? {
            synchronized(lock) {
                ...
                val file = File("$name/blockdata/c.${regionX},${regionZ}.bdb")
                if (file.exists()) {
                    ZipFile(file).use { zipFile ->
                        ByteArrayInputStream(zipFile.getInputStream(zipFile.getEntry("data")).readBytes()).use { byteArrayInputStream ->
        ...
        fun saveRegion(regionX: Int, regionZ: Int, unload: Boolean = false) {
            ...
            if (region != null) {
                ...
                if (region.blocks.isEmpty()) {
                    File("$name/blockdata/c.${regionX},${regionZ}.bdb").delete()
                } else {
                    try {
                        FileOutputStream(newFile("$name/blockdata/c.${regionX},${regionZ}.bdb")).use { fileOutputStream ->
